  ...
  <head>
	...
	<script type="text/javascript">
		...
		function f(){
			alert("123");
			$("#pid").change(function(){
				var str = $("#pid").find(option:selected).val();
    ...
    <form action="<c:url value='/admin/AdminCategoryServlet'/>" method="post" onsubmit="return checkForm()">
    	...
    	一级分类：<select name="pid" id="pid">
    		...
    		<c:forEach items="${parents }" var="parent">
    			    		<option class="select" value="${parent.cid }" <c:if test="${parent.cid eq child.parent.cid }">selected="selected"</c:if> >${parent.cname}</option>
    		</c:forEach> 
    	</select><br/>
